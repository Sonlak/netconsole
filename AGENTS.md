...
| 15 | Junos commit reliability | (a) `Junos cRPD` first commit after pool open spikes 20-30s on **NETCONF-over-SSH** (cold SSH session). For `interface_action` writes (shut/no-shut/set-access-vlan): measured on LAB-F2-AS-01 — NETCONF SSH cold-start: 27s then times out; RESTCONF load+commit: ~17s (fast, reliable). Write path is now RESTCONF-primary → NETCONF SSH fast-fallback (15s timeout) → SSH CLI. For `show-run` (read): RESTCONF ~2-5s. Backend watchdog: INTERFACE_ACTION on Juniper = 240s (120s base + 120s vendor extra). (b) If watchdog kills a mid-commit job, Junos keeps the half-loaded candidate. `worker/netconsole_worker/junos_rest.py::apply_set_configuration` auto-recovers on `configuration database modified` by `<discard-changes/>` + re-load+commit. (c) C-style `/* … */` in `DeviceSavedConfig.content` makes Junos emit `unknown command: /*` and leave a "modified" database — `backends/juniper.py::_set_commands` strips them and the backend `validateConfigPayload()` rejects the request with HTTP 400 before enqueue. (d) `SSHConnectionPool` has no `_create_conn` — `run_ssh_command` retry path must call `pool.borrow(...)` (fixed 2026-09-07, was a silent `AttributeError` that surfaced as "No existing session" in user logs). |
| 16 | Fabric port-index overlap across link direction | A DS port (e.g. `DS-01.bottom`) carries BOTH outgoing trunks (`DS→F1/F2/F3`, stored as `fromId=DS-01`) AND incoming trunks (`F4→DS-01`, stored as `fromId=F4`). The OLD code split port-index assignment between `sourcePortIdx` (from `linksBySource`) and `targetPortIdx` (from `linksByTarget`), so both the F1 outgoing and F4 incoming ended up at idx=0 → same X pixel → two lines visually emerge from the same stub, hiding one of the four uplinks at each DS. Fix: unified per-`(node, side)` incident map — collect ALL links touching `(node, side)`, sort by the OTHER endpoint's `box.x`, assign sequential idx 0..N-1. See `FabricDiagram.tsx::portIndex` and the `incident.forEach((id, i) => map.set(id, i))` block. Verified 2026-09-21 against LAB: F4-AS-01 is now a distinct port on F6-DS-01.bottom (X=432, idx=0) instead of being shadowed by F1-AS-01's stub. |
