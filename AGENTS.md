...
- User asked "sao kh�ng c� th�ng b�o ch?y push config xong v?y" � current
...
- Lessons for next agent:
  ...
  - **For copyright / version lines, prefer pure ASCII.** Any
    ...
    after save.
  - **NEVER pipe `Get-Content ... | Out-File` over a binary-heavy
    UTF-8 file.** I used it this session to "re-encode AGENTS.md as
    UTF-8" and lost 125 KB of session-log history (only the last
    2602 bytes survived). The fix: use `git checkout HEAD~1 --
    AGENTS.md` to restore, then append with Python (open in 'ab'
    mode with explicit utf-8 encoding) so the original byte stream
